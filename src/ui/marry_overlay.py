...
class MarryOverlay(Screen):
    def __init__(self, screen_manager, couple, marry_callback):
        ...
        self.__marry_button = Button(
            "Aceptar",
            size=(200, 25),
            position=(self.screen_manager.window.get_width() // 2 - 100, self.screen_manager.window.get_height() // 2),
            on_click=lambda: self.__marry(couple["id"], self.__inputs[0].get_text(), self.__inputs[1].get_text(), self.__handle_marry_error),
            bg_color=Colors.GREEN.value,
            font_size=16
        )
        ...
        self.__close_button = Button(
            "Cerrar",
            size=(200, 25),
            position=(self.screen_manager.window.get_width() // 2 - 100, self.screen_manager.window.get_height() // 2 + 75),
